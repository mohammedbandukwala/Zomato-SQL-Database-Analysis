...
SELECT 
	RESTAURANT_ID, 
	SUM(F.PRICE_PER_UNIT*OFD.QUANTITY) AS REVENUE
FROM ORDER_FOOD OFD 
JOIN FOODS F 
...
SELECT 
	OD.CUSTOMER_ID,
    AVG(OFD.QUANTITY*F.PRICE_PER_UNIT) AS AVG_ORDER_AMT
...
FROM (
	SELECT 
		...
		CUSTOMER_ID,
		COUNT(ORDER_ID) AS TOTAL_ORDERS,
		RANK() OVER	(PARTITION BY RESTAURANT_ID ORDER BY COUNT(ORDER_ID) DESC) AS RNK
		FROM ORDER_DETAIL
...
FROM ORDER_FOOD
GROUP BY RESTAURANT_ID
ORDER BY UNIQUE_FOODS DESC
LIMIT 1;

-- 15. Calculate total payment amount for each payment type.
SELECT 
	P.PAYMENT_TYPE,
    SUM(OFD.QUANTITY*F.PRICE_PER_UNIT) AS TOTAL_PAYMENT_AMT
FROM PAYMENT_TABLE P
JOIN ORDER_FOOD OFD
ON OFD.ORDER_ID=P.ORDER_ID
JOIN FOODS F 
ON F.FOOD_ID=OFD.FOOD_ID
